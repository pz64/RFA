        ...
        for (i in 0..<length) {
            ...
            if (decimationCounter == 0) {
                ...
                if (indexOut == outputCapacity) {
                    ...
                    outPacket.sampleRate =
                        inPacket.sampleRate / decimation // update the sample rate of the output sample packet
    ...
    companion object {
        ...
        fun createLowPassTaps(
            ...
            if (cutoffFrequency <= 0.0 || cutoffFrequency > sampleRate / 2) {
                Log.e(
                    LOGTAG,
                    "createLowPassTaps: firdes check failed: 0 < fa ($cutoffFrequency) <= sampling_freq ($sampleRate) / 2"
                )
                return null
            }
            ...
            for (n in -M..M) {
                if (n == 0) taps[n + M] =
                    fwT0 / Math.PI.toFloat() * windowFunction.value(n + M, ntaps)
                else {
                    // a little algebra gets this into the more familiar sin(x)/x form
                    taps[n + M] =
                        sin((n * fwT0).toDouble()).toFloat() / (n * Math.PI.toFloat()) * windowFunction.value(
                            n + M,
                            ntaps
                        )
                }
