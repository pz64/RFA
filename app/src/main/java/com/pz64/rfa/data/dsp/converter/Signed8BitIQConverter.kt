 ...
 */
class Signed8BitIQConverter : IQConverter() {
    override fun generateLookupTable() {
        ...
        for (i in 0..255) lookupTable!![i] = (i - 128) / 128.0f
    }
    ...
    override fun fillPacketIntoInterleavedBuffer(
        ...
        for (i in packet.indices) {
            interleavedBuffer[i] = lookupTable!![packet[i] + 128]
        }
        return true
    }
    ...
    override fun mixPacketIntoSamplePacket(
        ...
        while (i < packet.size) {
            re[startIndex + count] =
                cosineRealLookupTable!![cosineIndex]!![packet[i] + 128] - cosineImagLookupTable!![cosineIndex]!![packet[i + 1] + 128]
            im[startIndex + count] =
                cosineRealLookupTable!![cosineIndex]!![packet[i + 1] + 128] + cosineImagLookupTable!![cosineIndex]!![packet[i] + 128]
        ...
        samplePacket.setSize(samplePacket.size() + count) // update the size of the sample packet
        samplePacket.sampleRate = sampleRate // update the sample rate
        samplePacket.frequency = channelFrequency // update the frequency
        return count
    }
}
