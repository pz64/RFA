...
class Demodulator(
    ...
    companion object {
        ...
        const val CW_OFFSET_FREQUENCY =
            750  // Offset for the CW signal. 750Hz will be the audio tone the user hears if the signal is exactly at the channel frequency
    }
    ...
    var channelWidth: Int = 0
        set(value) {
            field =
                value.coerceIn(demodulationMode.minChannelWidth, demodulationMode.maxChannelWidth)
        }
    ...
    private val resampler = Resampler(
        demodulationMode.quadratureRate,
        packetSize,
        inputQueue,
        inputReturnQueue,
        (filterQuality * 500).toInt()
    )

    // AUDIO OUTPUT
    private var audioSink: AudioSink = AudioSink(
        packetSize,
        AUDIO_RATE,
        lowPowerformanceMode
    ) // Will do QUADRATURE_RATE --> AUDIO_RATE and audio output
    ...
    suspend fun start() = coroutineScope {
        ...
        Log.i(LOGTAG, "Demodulator started.")

        // Start resampler coroutine:
        launch { resampler.run() }

        // Start audio sink coroutine:
        launch { audioSink.run() }

        try {
            while (isActive) {
                // Get downsampled packet from the resampler:
                inputSamples = withContext(Dispatchers.IO) {
                    resampler.getResampledPacket(1000)
                }
                ...
                // return input samples to the resampler block:
                ...
                audioBuffer = withContext(Dispatchers.IO) {
                    audioSink.getPacketBuffer(1000)
                }
                ...
                when (demodulationMode) {
                    ...
                    DemodulationMode.NFM -> demodulateFM(
                        quadratureSamples,
                        audioBuffer,
                        maxDeviation = channelWidth * 0.75f
                    )

                    DemodulationMode.WFM -> demodulateFM(
                        quadratureSamples,
                        audioBuffer,
                        maxDeviation = channelWidth * 0.85f
                    )
            ...
            // resampler and audioSink jobs are cancelled automatically because of coroutineScope
            Log.i(LOGTAG, "Demodulator stopped.")
        }
    }
    ...
    private fun applyUserFilter(input: SamplePacket, output: SamplePacket) {
        ...
        if (userFilter == null || (userFilter!!.cutOffFrequency.toInt()) != channelWidth) {
            ...
            Log.d(
                LOGTAG,
                ("applyUserFilter: created new user filter with " + userFilter!!.numberOfTaps
    ...
    private fun demodulateFM(input: SamplePacket, output: SamplePacket, maxDeviation: Float) {
        ...
        val quadratureGain =
            demodulationMode.quadratureRate / (2 * Math.PI * maxDeviation).toFloat()
