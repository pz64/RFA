...
class BlackmanWindow : WindowFunction {
    override fun value(n: Int, N: Int): Float {
        return (0.42f
                - 0.5f * cos(2.0 * Math.PI * n / (N - 1)).toFloat()
                + 0.08f * cos(4.0 * Math.PI * n / (N - 1)).toFloat())
    }
...
class HammingWindow : WindowFunction {
    override fun value(n: Int, N: Int): Float {
        return (0.54f - 0.46f * cos(2.0 * Math.PI * n / (N - 1)).toFloat())
    }
...
class KaiserWindow(private val beta: Double) : WindowFunction {
    ...
    override fun value(n: Int, N: Int): Float {
        ...
        val valN = izero(beta * sqrt(1.0 - temp * temp)) * iBeta
        return valN.toFloat()
    }
